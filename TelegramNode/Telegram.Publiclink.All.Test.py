...
1. 支持多级阶梯测速模式（TCP/Clash/XC Speedtest 自由组合）。
2. 集成 Cloudflare Warp 智能网络切换，模拟国内真实测速环境。
3. 新增针对超级反爬机场的浏览器级绕过下载功能。
4. 引入节点质量评分系统（0-100分）及四级质量标签标注。
5. 支持基于实测带宽（MB/s）的节点硬性阈值筛选。
6. 增强 Shadowsocks 2022 协议密码的字节级严格校验与修复。
7. 实现节点名称全局归一化重写（国旗+地区+序号+质量标识）。
8. 支持分阶段保存测速中间件文件（TCP/Clash/Speedtest.yaml）。
9. 优化 Telegram 抓取逻辑，支持自定义时间窗口消息过滤。
10. 完善 Hysteria/Hysteria2 协议配置的自动补全与非法参数清洗。
11. 新增 GitHub 专用路由策略，防止 Actions 运行过程中网络断连。
"""
import os
import re
import sys
