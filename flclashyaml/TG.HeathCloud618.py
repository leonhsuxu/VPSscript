        ...
        try:
            ...
            try:
                ...
                if isinstance(data, dict) and 'proxies' in data:
                    print(f"  ✓ 成功获取 {len(data['proxies'])} 个节点")
                    return data['proxies']
                ...
                try:
                    ...
                    if isinstance(data, dict) and 'proxies' in data:
                        print(f"  ✓ 成功获取 {len(data['proxies'])} 个节点 (Base64)")
                        return data['proxies']
                except Exception:
                    pass
            ...
            if e.response.status_code == 403:
                print(f"  ✗ 403 错误 - 访问被拒绝，等待 {RETRY_DELAY} 秒后重试...")
            else:
                print(f"  ✗ HTTP 错误 {e.response.status_code}")
        except requests.exceptions.Timeout:
            print(f"  ✗ 请求超时，等待 {RETRY_DELAY} 秒后重试...")
        except requests.exceptions.ConnectionError:
            print(f"  ✗ 连接错误，等待 {RETRY_DELAY} 秒后重试...")
        except Exception as e:
            print(f"  ✗ 下载失败: {e}")
            
    print(f"  ✗ {url[:50]}... 下载失败，已重试 {MAX_RETRIES} 次")
    return []
